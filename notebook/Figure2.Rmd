...
```{r}
Immune.Cell.TrueV2.sel <- subset(Immune.Cells.TrueV2, subset = Class %in% "Peritoneal endometriosis")


#rearrange groups by proportions
Immune.Cell.TrueV2.sel@meta.data$Immune_Cluster_ID <- factor(Immune.Cell.TrueV2.sel@meta.data$Immune_Cluster_ID, 
...
Immune.Cell.TrueV2.sel@meta.data %>% 
      group_by(Immune_Cluster_ID) %>% 
     summarise(n_tissue=n_distinct(orig.ident), n_patient= n_distinct(Patient.No.), n=n()) %>%
      print(n=29)
...
Immune.Cell.TrueV2.sel@meta.data <- droplevels(Immune.Cell.TrueV2.sel@meta.data)

counts.prop = data.frame(table(Immune.Cell.TrueV2.sel@meta.data$LA_Status, Immune.Cell.TrueV2.sel@meta.data$Immune_Cluster_ID))
counts.prop.perc = group_by(counts.prop, Var2) %>% mutate(percent = Freq/sum(Freq))
head(counts.prop.perc)
aux = aggregate(counts.prop$Freq, by=list(Category=counts.prop$Var1), FUN=sum)
aux$Var2 = "Total"
...
write_csv(Immune.Cell.TrueV2.sel@meta.data,"data/Endometriosis.meta.data.csv")
...
table.class.cell = table(Immune.Cells.TrueV2_sel@meta.data$Immune_Cluster_ID, Immune.Cell.TrueV2.sel@meta.data$LA_Status)
table.class.cell.perc = table.class.cell/rowSums(table.class.cell)

obs.class.perc = table(Immune.Cell.TrueV2.sel@meta.data$LA_Status)/length(Immune.Cell.TrueV2.sel@meta.data$LA_Status) #expected proportion of high and low in entire data
...
counts.prop = data.frame(table(Immune.Cells.TrueV2_sel@meta.data$Stage, Immune.Cell.TrueV2.sel@meta.data$Immune_Cluster_ID))
counts.prop.perc = group_by(counts.prop, Var2) %>% mutate(percent = Freq/sum(Freq))
head(counts.prop.perc)
aux = aggregate(counts.prop$Freq, by=list(Category=counts.prop$Var1), FUN=sum)
aux$Var2 = "Total"
...
Immune.Cell.TrueV2.sel@meta.data <- Immune.Cell.TrueV2.sel@meta.data  %>%
...
counts.prop = data.frame(table(Immune.Cell.TrueV2.sel@meta.data$Stage_Group, Immune.Cell.TrueV2.sel@meta.data$Immune_Cluster_ID))
counts.prop.perc = group_by(counts.prop, Var2) %>% mutate(percent = Freq/sum(Freq))
head(counts.prop.perc)
aux = aggregate(counts.prop$Freq, by=list(Category=counts.prop$Var1), FUN=sum)
aux$Var2 = "Total"
...
```{r}
Endometriosis_Deep_Sup <- subset(Immune.Cell.TrueV2.sel, subset = Deep_Superficial %in% c("Superficial", "Deep"))
...
#NK cell analysils
```{r}
Endometriosis_sel <- subset(Immune.Cell.TrueV2.sel, subset = Immune_Cluster_ID %in% "NK cells")
...
#Macs analysis
```{r}
Endometriosis_sel <- subset(Immune.Cell.TrueV2.sel, subset = Immune_Cluster_ID %in% c("Macrophage M1/M2", "Macrophages M1", "Macrophages Me"))
...
```{r}
immue_endo_t<- Immune.Cell.TrueV2.sel@meta.data %>%
...
anno_df <- Immune.Cell.TrueV2.sel@meta.data  %>%
           rownames_to_column() %>%
