...
immue_endo_t<- Endometriosis@meta.data %>%
                  ...
                  group_by(SampleName,seurat_clusters) %>%
...
heatmap_df <- immue_endo_t  %>% 
              dplyr::select(seurat_clusters,prop,SampleName) %>%
              spread(seurat_clusters, prop) %>%
              column_to_rownames("SampleName") %>%
              as.matrix() %>%
              replace(.,is.na(.),0) %>%
              t()
...
inner_join(anno_df %>% rownames_to_column(".rownames"), by=".rownames") %>%
  ggplot(aes(x=.fittedPC1, y=.fittedPC2,color=LA_Status,label=Patient.No.))+
  geom_point(size=5) +
