...
excel_path <- "data/Clusters_Major_Groups.xlsx"
sheets <- readxl::excel_sheets(excel_path)
MajorGroups <- sheets %>% 
                      lapply(. , function(s){ read_excel(excel_path, sheet = s) %>%
                                              mutate(across(everything(),as.factor))}) %>%
...
d10x.seurat@meta.data <- d10x.seurat@meta.data %>%
              rownames_to_column() %>%
              left_join(MajorGroups$Sheet1, by=c("SCT_snn_res.3"="Cluster_Number")) %>%
  column_to_rownames()
```
...
#can continue from here on

#want to get Immune proportion by high vs low in peritoneal endometriosis
```{r}
#proportions by LA tissue activity
Endometriosis <- subset(d10x.seurat, subset = Class %in% "Peritoneal endometriosis")

Endometriosis@meta.data <- droplevels(Endometriosis@meta.data)

counts.prop = data.frame(table(Endometriosis@meta.data$Major_groups, Endometriosis@meta.data$LA_Status))
counts.prop.perc = group_by(counts.prop, Var2) %>% mutate(percent = Freq/sum(Freq))
head(counts.prop.perc)
aux = aggregate(counts.prop$Freq, by=list(Category=counts.prop$Var1), FUN=sum)
aux$Var2 = "Total"
...
p1 <- ggplot(counts.prop.perc, aes(x = Var2, y = percent, fill = Var1)) + 
  geom_bar(position="stack",stat = "identity", width=0.8) +
  scale_fill_manual(name="LA Group", values = c("Immune cells" = "#d73027",
                                                "Endothelial cells" = "#f46d43",
                                                "EnEpi cells" = "#fdae61",
                                                "Ens cells" = "#fee090",
                                                "Epithelial cells" = "#e0f3f8",
                                                "Erythrocytes" = "#abd9e9",
                                                "Mesenchymal cells" = "#74add1",
                                                "Smooth muscle cells" = "#4575b4")) +
  scale_x_discrete(limits = rev(levels(counts.prop.perc$Var2))) +
  coord_flip() +
  theme_set(theme_classic(base_size = 30)) +
  theme(plot.title = element_text(hjust = 0.5),
