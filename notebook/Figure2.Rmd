...
d10x.seurat <- readRDS("fixed_annotated_aux.seurat.rds")
```
...
Immune.Cells.TrueV2 <- FindClusters(Immune.Cells.TrueV2, resolution = 0.9)
...
p1 <- DimPlot(Immune.Cells.TrueV2, reduction = "umap", label =  TRUE, raster = FALSE) + 
  theme_classic(base_size = 20) +
  ggtitle('UMAP of Immune cells TrueV2 subclusters Res 0.9')
...
p1 <-DotPlot(Immune.Cells.TrueV2, assay = "RNA", features = selected.features) + labs(title = "Identifying Immune cell TrueV2 Sub-cluster by Canonical Markers res0.9") +
 theme_classic(base_size = 20) +
  theme(axis.text.x = element_text(angle = 90, vjust = 0.5, hjust = 0.8), axis.text.y = element_text(size = 9)) +
  coord_flip() +
  scale_colour_gradient2(low = "#2166ac", mid = "#f7f7f7", high = "#b2182b")
...
ggplot(counts.prop.perc, aes(x = Var2, y = percent, fill = Var1)) + 
  ...
  ggtitle('Proportion of Immune cells TrueV2 by patient res0.7')
```
...
p1 <- ggplot(counts.prop.perc, aes(x = Var2, y = percent, fill = Var1)) + 
  ...
  theme(plot.title = element_text(hjust = 0.5),
        ...
        axis.title.x = element_blank(),
        axis.title.y = element_blank()) +
  ggtitle('Proportion of Immune cells TrueV2 res0.47 in LA Status') +
  theme_classic(base_size = 20)
...
counts.prop = data.frame(table(Endometriosis@meta.data$Major_groups, Endometriosis@meta.data$Patient_LA_Status_Tissue_Level))
counts.prop.perc = group_by(counts.prop, Var2) %>% mutate(percent = Freq/sum(Freq))
head(counts.prop.perc)
aux = aggregate(counts.prop$Freq, by=list(Category=counts.prop$Var1), FUN=sum)
aux$Var2 = "Total"
...
p1 <- ggplot(counts.prop.perc, aes(x = Var2, y = percent, fill = Var1)) + 
  geom_bar(position="stack",stat = "identity", width=0.8) +
  scale_fill_manual(name="Major Groups", values = c("Immune cells" = "#d73027",
